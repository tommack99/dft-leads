...
export default async function handler(req,res){
  ...
  do{
    ...
    const dr=await fetch("https://api.monday.com/v2",{method:"POST",headers:{"Content-Type":"application/json","Authorization":MON},body:JSON.stringify({query:q})});
    const dd=await dr.json();
    const page=dd&&dd.data&&dd.data.boards&&dd.data.boards[0]&&dd.data.boards[0].items_page;
    if(!page)break;
    items=items.concat(page.items||[]);
    cursor=page.cursor||null;
  }while(cursor);
  const er=await fetch("https://api.monday.com/v2",{method:"POST",headers:{"Content-Type":"application/json","Authorization":MON},body:JSON.stringify({query:"{boards(ids:["+REN+"]){items_page(limit:500){items{name}}}}"})});
  ...
  for(const item of items){
    ...
    for(const sub of(item.subitems||[])){
      ...
      let draft="";
      try{
        const prompt="Draft a short renewal email from Margot at Digital Fox Talent to a brand contact. Warm, friendly, under 150 words, no em dashes. Campaign: "+item.name+". Advertiser: "+advertiser+". Creator: "+creator+". Client price: $"+clientPrice.toFixed(0)+". Renewal (50% off): $"+renewal+"."+(vn?" Views: "+views+". eCPM: $"+ecpm+".":"")+" Live: "+liveStr+". Pitch the repost. Table: Creator|Original|Renewal|Views|eCPM. Start: Hi [name],";
      ...
      const cols=JSON.stringify({text_mm3t6g60:advertiser,text_mm3tacsw:creator,numeric_mm3tqsff:Math.round(clientPrice),numeric_mm3t15r1:renewal,numeric_mm3tbpvz:vn,numeric_mm3ty8e:ecpm,date_mm3t55tj:{date:liveStr},long_text_mm3tm4wc:{text:fullDraft}});
      try{
        const sr=await fetch("https://api.monday.com/v2",{method:"POST",headers:{"Content-Type":"application/json","Authorization":MON},body:JSON.stringify({query:"mutation{create_item(board_id:"+REN+",group_id:"+JSON.stringify(GRP)+",item_name:"+JSON.stringify(iname)+",column_values:"+JSON.stringify(cols)+"){id}}"})});
        const sd=await sr.json();
        if(sd&&sd.data&&sd.data.create_item&&sd.data.create_item.id)added++;
      }catch(e){}

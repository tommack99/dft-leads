...
export default async function handler(req,res){
  try{
    ...
    for(const grpId of DONE_GROUPS){
      ...
      do{
        const q=cursor
          ?("{boards(ids:["+CAM+"]){groups(ids:[\""+grpId+"\"]){items_page(limit:50,cursor:"+JSON.stringify(cursor)+"){cursor items{id name column_values(ids:[\"date_mm1mcn87\",\"dropdown_mm1a3tqp\",\"date__1\"]){id text value}subitems{id name column_values(ids:[\"numbers\",\"numeric_mm3vg42g\",\"numeric_mm3vrpfr\",\"timerange_mm1m50vx\"]){id text value}}}}}}}")
          :("{boards(ids:["+CAM+"]){groups(ids:[\""+grpId+"\"]){items_page(limit:50){cursor items{id name column_values(ids:[\"date_mm1mcn87\",\"dropdown_mm1a3tqp\",\"date__1\"]){id text value}subitems{id name column_values(ids:[\"numbers\",\"numeric_mm3vg42g\",\"numeric_mm3vrpfr\",\"timerange_mm1m50vx\"]){id text value}}}}}}}");
    ...
    for(const item of items){
      ...
      for(const sub of(item.subitems||[])){
        ...
        toProcess.push({advertiser,creator:sub.name,clientPrice,renewal,iname,views,vn,ecpm,liveStr:subLiveStr||""});
      }
    }
    let added=0;
    for(const p of toProcess){
      let draft="";
      try{
        const prompt="Short renewal email from Margot at Digital Fox Talent. Warm, under 150 words, no em dashes. Advertiser: "+p.advertiser+". Creator: "+p.creator+". Price: $"+Math.round(p.clientPrice)+". Renewal: $"+p.renewal+"."+(p.vn?" Views: "+p.views+". eCPM: $"+p.ecpm+".":"")+" Live: "+p.liveStr+". Pitch the repost. Table: Creator|Original|Renewal|Views|eCPM. Start: Hi [name],";
      ...
      const cols2=JSON.stringify({text_mm3t6g60:p.advertiser,text_mm3tacsw:p.creator,numeric_mm3tqsff:Math.round(p.clientPrice),numeric_mm3t15r1:p.renewal,numeric_mm3tbpvz:p.vn,numeric_mm3ty8e:p.ecpm,date_mm3t55tj:{date:p.liveStr||"2025-01-01"},long_text_mm3tm4wc:{text:fullDraft}});

...
export default async function handler(req,res){
  ...
  async function monday(query){
    const r=await fetch("https://api.monday.com/v2",{method:"POST",headers:{"Content-Type":"application/json","Authorization":MON},body:JSON.stringify({query})});
    return r.json();
  }

  // Fetch campaigns with subitems
  let items=[];
  let cursor=null;
  do{
    const cols="deal_value,status,date4,text_mm1zsbnp";
    const scols="numbers,text_mm3ta8gs,file_mm1zd4v9";
    const q=cursor
      ?mq("{boards(ids:["+CAM+"]){items_page(limit:50,cursor:"+JSON.stringify(cursor)+"){cursor items{id name column_values(ids:"+JSON.stringify(cols.split(","))+"){id text value}subitems{id name column_values(ids:"+JSON.stringify(scols.split(","))+"){id text value}}}}}}")
      :mq("{boards(ids:["+CAM+"]){items_page(limit:50){cursor items{id name column_values(ids:"+JSON.stringify(cols.split(","))+"){id text value}subitems{id name column_values(ids:"+JSON.stringify(scols.split(","))+"){id text value}}}}}}");
  ...
  const ed=await monday("{boards(ids:["+REN+"]){items_page(limit:500){items{name}}}}");
  ...
  for(const item of items){
    ...
    if(!liveStr)continue;
    if((now-new Date(liveStr))/(1000*60*60*24*7)<1)continue;
    ...
    for(const sub of(item.subitems||[])){
      ...
      try{
        const prompt=[
          "Draft a renewal outreach email from Margot at Digital Fox Talent (DFT) to a brand contact.",
          "Warm, friendly, professional. Under 150 words. No em dashes.",
          "Campaign: "+item.name+". Advertiser: "+advertiser+". Creator: "+creator+".",
          "Original client price: $"+clientPrice.toFixed(0)+". Renewal offer (50% off): $"+renewal+".",
          vn?"Views: "+views+". eCPM: $"+ecpm+".":"",
          "Live date: "+liveStr+".",
          "Pitch the 50% repost offer. Include table: Creator | Original | Renewal | Views | eCPM",
      ...
      const cols={text_mm3t6g60:advertiser,text_mm3tacsw:creator,numeric_mm3tqsff:clientPrice,numeric_mm3t15r1:renewal,numeric_mm3tbpvz:vn,numeric_mm3ty8e:ecpm,date_mm3t55tj:{date:liveStr},long_text_mm3tm4wc:{text:fullDraft}};
      const mutation="mutation{create_item(board_id:"+REN+",group_id:"+JSON.stringify(GRP)+",item_name:"+JSON.stringify(iname)+",column_values:"+JSON.stringify(JSON.stringify(cols))+"){id}}";
      try{
        const sr=await monday(mutation);
  ...
  return res.json({message:"Renewal scan complete",campaignsScanned:items.length,renewalsAdded:added});
}

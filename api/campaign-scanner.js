...
export default async function handler(req,res){
  ...
  for(var i=0;i<items.length;i++){
    ...
    for(var j=0;j<subs.length;j++){
      ...
      var draft="";
      try{
        var prompt="Draft a short renewal email from Margot at Digital Fox Talent to a brand. Warm, friendly, under 150 words, no em dashes. Advertiser: "+advertiser+". Creator: "+sub.name+". Client price: $"+Math.round(clientPrice)+". Renewal: $"+renewal+"."+(vn?" Views: "+views+". eCPM: $"+ecpm+".":"")+" Live: "+liveStr+". Pitch the repost. Table: Creator|Original|Renewal|Views|eCPM. Start: Hi [name],";
      ...
      try{
        var sr=await mon(MON,"mutation{create_item(board_id:"+REN+",group_id:"+JSON.stringify(GRP)+",item_name:"+JSON.stringify(iname)+",column_values:"+JSON.stringify(cols)+"){id}}");
